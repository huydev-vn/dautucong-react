...
export const hopDongApi = {
  getList: async (params?: HopDongListParams): Promise<PaginatedResponse<HopDong>> => {
    const { data } = await axiosInstance.get<ApiResponse<PaginatedResponse<HopDong>>>(BASE, { params });
    return data.data;
  },

  getById: async (id: string): Promise<HopDong> => {
    const { data } = await axiosInstance.get<ApiResponse<HopDong>>(`${BASE}/${id}`);
    return data.data;
  },

  create: async (payload: Omit<HopDong, 'id' | 'ngayTao' | 'ngayCapNhat'>): Promise<HopDong> => {
    const { data } = await axiosInstance.post<ApiResponse<HopDong>>(BASE, payload);
    return data.data;
  },

  update: async (id: string, payload: Partial<HopDong>): Promise<HopDong> => {
    const { data } = await axiosInstance.put<ApiResponse<HopDong>>(`${BASE}/${id}`, payload);
    return data.data;
  },
